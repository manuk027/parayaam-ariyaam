                ...
                <div className="flex items-center items-center gap-3">
                    {user ? (
                        <div className="hidden md:flex items-center gap-3">
                            ...
                            <button onClick={handleLogout} className="p-2 rounded-full hover:bg-gray-200 transition" title="Logout">
                                <LogOut size={18} />
                            </button>
                        </div>
                    ) : (
                        <div className="hidden md:flex items-center gap-3">
                            ...
                            <Link to="/signup" className="bg-slate-900 text-white px-5 py-2 rounded-full text-sm font-bold hover:bg-green-600 transition shadow-lg shadow-slate-200">Sign up</Link>
                        </div>
                    )}
                    <button onClick={toggleMenu} className="md:hidden p-2 text-slate-700 hover:text-green-600 transition-colors" aria-label="Toggle Menu">
                        {isOpen ? (
            ...
            <div className={`
                ...
                <div className="flex flex-col p-6 gap-4 text-center font-bold text-slate-700 uppercase tracking-widest text-sm">
                    ...
                    {user ? (
                        ...
                        <div className="flex flex-col gap-3 flex items-center justify-center">
                            <span onClick={toggleMenu} className="text-green-600 py-2">{user.displayName}</span>
                            <button onClick={handleLogout} className="bg-slate-900 text-white p-3 rounded-3xl flex items-center justify-center w-10" ><LogOut size={17} /></button>
                        </div>
                    ) : (
